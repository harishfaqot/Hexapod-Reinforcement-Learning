...
def _build_actuator_map(model, nu):
    ...
    for leg in LEG_ORDER:
        ...
        for jnt in ["coxa", "femur", "tibia"]:
            found = next((name_to_idx[n] for n in [f"{jnt}_{leg}", f"{leg}_{jnt}", f"{jnt}_{leg}_motor", f"{leg}_{jnt}_motor"] if n in name_to_idx), None)
...
def _compute_neutral_ik():
    ...
    for i, leg in enumerate(LEG_ORDER):
        base = np.zeros(3, dtype=np.float32)  # body_kinematics((0,0,0),(0,0,0))[i]
...
def compute_gait_ctrl(data, calib, t, vx, vy, v_rot, step_height, duty, cpg_hz, phase_lag, body_pos=(0,0,0), body_orient=(0,0,0)):
...
# ─── HexapodEnv (combines sim + RL gym interface) ─────────────────────────────

class HexapodEnv(gym.Env):
    """Hexapod MuJoCo env. Works as a direct sim (HexapodSimple-style) or RL gym env."""
    ...
    def __init__(
        ...
        # RL action: [step_height, duty, x, y, z, roll, pitch, yaw]
    ...
    def step(self, action):
        ...
        return obs, reward, terminated, truncated, {"vcmd_xy": self.vcmd_xy.copy(), "stuck_detected": bool(stuck)}
...
class HexapodTkUI:
    """Tkinter UI for direct IK testing. Can also be attached to HexapodEnv for RL+UI."""
    ...
    def __init__(self, env: HexapodEnv):
        ...
        self.leg_offset_controls = {}   # {leg: {x, y, z}}
        self.joint_deg_controls = {}    # {leg: {coxa, femur, tibia}}
        self.control_mode = tk.StringVar(value="ik")
        self.camera_follow_var = tk.BooleanVar(value=True)

        self._build_ui()
        self.tick_ms = 8; self.tick_count = 0
    ...
    def _build_ui(self):
        ...
        for t in [t1, t2, t3]: t.grid_columnconfigure(0, weight=1); t.grid_columnconfigure(1, weight=1)

        # ── Movement tab ──
        tk.Label(t1, text="Movement controls", font=("Arial", 11, "bold")).grid(row=0, column=0, columnspan=2, padx=8, pady=(8,4), sticky="w")
        ...
        tk.Label(t1, text="Body controls", font=("Arial", 11, "bold")).grid(row=0, column=1, columnspan=2, padx=8, pady=(8,4), sticky="w")
        ...
        # IMU plots
        tk.Label(t1, text="IMU telemetry", font=("Arial", 11, "bold")).grid(row=10, column=0, columnspan=2, padx=8, pady=(10,4), sticky="w")
        imu_f = tk.Frame(t1); imu_f.grid(row=11, column=0, columnspan=2, padx=8, pady=(0,6), sticky="we")
        ...
        if Figure is not None:
            for col, (names, colors, yr, attr) in enumerate([
                ...
                p = _IMUPlot(f, names, colors, yr)
                p.widget.pack(fill="both", expand=True)
                setattr(self, attr, p)
        ...
        # ── Foot params tab ──
        tk.Label(t3, text="Foot home tuning", font=("Arial",11,"bold")).grid(row=0, column=0, columnspan=2, padx=8, pady=(8,4), sticky="w")
        ...
        if self.control_mode.get() == "joint":
            for leg in LEG_ORDER:
                for jnt in ["coxa","femur","tibia"]:
                    c = calib[leg][jnt]
                    if c is None: continue
                    target = _clamp(c["neutral_joint_qpos"] + np.radians(float(self.joint_deg_controls[leg][jnt].get())) * c["delta_sign"], c["low"], c["high"])
            ...
            ctrl = compute_gait_ctrl(
                ...
                body_pos=(float(self.x_slider.get()), float(self.y_slider.get()), float(self.z_slider.get())),
                body_orient=(float(self.r_slider.get()), float(self.p_slider.get()), float(self.yaw_slider.get())),
            )
            # Apply per-leg IK offsets
            body_leg_pos = body_kinematics(
                (float(self.x_slider.get()), float(self.y_slider.get()), float(self.z_slider.get())),
                (float(self.r_slider.get()), float(self.p_slider.get()), float(self.yaw_slider.get())),
            )
            for i, leg in enumerate(LEG_ORDER):
    ...
    def _on_close(self):
        self.env.close(); self.root.destroy()
...
if __name__ == "__main__":
    env = HexapodEnv(randomize_spawn=False)
    env.reset()
    print(f"HexapodEnv ready | nu={env.nu}")
    ui = HexapodTkUI(env)
    env.ui = ui   # attach UI to env so RL step can read cpg/heading sliders
    ui.run()